...
namespace Client_WPF.ViewModels
{
    public class ApplicationViewModel : INotifyPropertyChanged
    {
        ...
        private Caching<HotelInfo> Cache { get; set; }
        public ObservableCollection<HotelInfo> CacheList { get; set; }

        public ObservableCollection<HotelInfo> Favorites { get; set; }
                ...
                return downCommand ??
                  (downCommand = new RelayCommand(obj =>
                  {
                      ...
                      if (sizeList + 10 >= dbSize)
                      {
                          ...
                          {
                              Hotels.Add(db.Hotels.Include(s => s.ContactPhone).FirstOrDefault(p => p.Id == i));
                          }
                      }
                      else
                      {
        ...
        public RelayCommand AddCommand
        {
            get
            {
                return addCommand ??
                  ...
                  {
                      HotelInfo elem = obj as HotelInfo;

                      if (elem != null && elem.Id != 0)
                      {
                          ReadWriter<List<HotelInfo>>.Write(new List<HotelInfo>() { elem });
                      }
                      else if (elem != null)
                      {
        ...
        public RelayCommand UpdateCommand
        {
            get
            {
                return updateCommand ??
                  ...
                  {
                      List<HotelInfo> temp_list = JDeserializer<List<HotelInfo>>.Deser(ReadWriter<List<HotelInfo>>.Read());
                      ...
                      if (temp_list != null)
                      {
                          ...
                          {
                              bool found;
                              foreach (HotelInfo itemT in temp_list)
                              {
                                  found = false;

                                  foreach (var itemD in Favorites)
                                  {
                                       if (itemD.FullName == itemT.FullName)
                                        found = true;
                                  }
